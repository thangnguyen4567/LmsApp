 ...
 *
 * ⚠️ Hai thứ đó dùng MỘT LẦN cho phiên hiện tại, KHÔNG lưu xuống MMKV: lần mở
 * app sau sẽ nạp lại `sid` đã hết hạn, mà backend ưu tiên `sid` hơn `user_saas`
 * nên rớt về trang đăng nhập dù phiên SaaS còn tốt.
 *
 * Định danh app AMIS (scheme, package, endpoint trang QL…) nằm ở
 * src/services/amisConfig.js.
 */

// Tài liệu tích hợp viết là "TenantID, SID" nên không chắc AMIS gửi hoa hay
// thường -> mọi thao tác đọc/xoá đều so khớp không phân biệt hoa thường.
const AMIS_PARAMS = ['sid', 'tenantid', 'lang', 'userid'];

// AMIS gửi ngôn ngữ lạ -> ép về 'en' (không phải 'vi'): coi như người dùng không
// thuộc nhóm tiếng Việt thì tiếng Anh dễ hiểu hơn.
const SUPPORTED_LANGS = ['vi', 'en'];
const FALLBACK_LANG = 'en';

/**
 * Deep link này có phải do app AMIS mở không.
 * Căn cứ: có mang tham số định danh (`sid`/`tenantid`/`userid`). `lang` KHÔNG
 * tính — nó chỉ là tuỳ chọn hiển thị, không định danh nguồn gọi.
 */
...
// Điểm vào đăng nhập thường. `applms=true` là cờ web dùng để biết request đến từ
// app — giữ đúng dạng mà luồng nhập link tay đang lưu.
...
// không dùng tới phần suy đoán dưới đây.
//
// LMS có thể cài ở GỐC domain (khách hàng thường) hoặc dưới SUB-PATH (MISA JSC
// dùng /lms) nên tuyệt đối không hard-code sub-path. Cách suy: cắt tại thư mục
// cấp 1 của Moodle, phần đứng trước chính là wwwroot.
...
// thư mục chỉ có khi dev vì không bao giờ xuất hiện trong URL người dùng.
...
// Link cũ của MISA JSC — giữ để deep link phiên bản AMIS cũ (không kèm sid) chạy
// y như trước.
 ...
 * '' (giữ nguyên ngôn ngữ app) · 'vi'/'en' (nhận, chấp cả 'vi-VN') · khác -> 'en'
 */
...
/**
 * Tách `sid`/`tenantid`/`lang`/`userid` khỏi URL deep link. Trả
 * `{cleanUrl, sid, tenantid, lang, userid}` — `cleanUrl` là bản an toàn để lưu
 * lâu dài. URL sai định dạng hoặc không mang tham số nào thì trả lại nguyên bản
 * (không chuẩn hoá lại chuỗi) để deep link cũ giữ nguyên hành vi.
 *
 ...
 * - `tenantid`, `userid`: gắn với người dùng/đơn vị của phiên đó.
 * - `lang`: để nằm lại thì ContentView luôn tôn trọng giá trị cũ, người dùng đổi
 *   ngôn ngữ trong app sẽ không có tác dụng (ngôn ngữ nhớ qua `app_locale`).
 */
...
/**
 * Đọc `sid`/`tenantid` từ params của deep link. AMIS có thể đặt chúng ở một
 * trong hai chỗ, hỗ trợ cả hai:
...
/**
 * Đọc `sid` từ URL sắp nạp. Backend còn nhận `sid` qua cookie `x-sessionid` nên
 * cần chính giá trị, không chỉ cần biết có hay không.
 */
...
/** Đọc `tenantid` từ URL sắp nạp — nguồn giá trị cho cookie `TENANT`. */
...
/**
 * URL sắp nạp có mang `sid` không. Quyết định body POST: có `sid` thì `sid` là
 * danh tính duy nhất, không gửi kèm username/password/user_saas.
 */
...
/**
 * Suy wwwroot (gốc site LMS) từ một URL bất kỳ trong site đó. Chạy được cho cả
 * site cài ở gốc domain lẫn site dưới sub-path. Trả '' nếu URL không phân tích được.
 */
export function deriveWwwroot(rawUrl) {
    ...
    // 4); bắt trúng '/user/' (vị trí 17) thì wwwroot ra '/lms/grade/report'.
    let cutAt = -1;
 ...
 *
 * KHÔNG hard-code host hay sub-path: AMIS có bản cho MISA JSC (site dưới `/lms`)
 * và bản cho khách hàng khác (domain riêng, thường cài ở gốc). wwwroot suy từ
 * chính URL nhận được.
 *
 ...
 *
 * Hai điểm vào nhận hai loại thông tin khác nhau: `auth/saas/index.php` chỉ hiểu
 * `sid`/`user_saas` và **bỏ qua** `username`/`password`, còn `login/index.php`
 * mới là chỗ nhận `username`/`password`. Đăng nhập tay xong mà vẫn giữ điểm vào
 * SaaS thì lần mở app sau bị đá sang MISA, dù máy đang có đủ tài khoản thường.
 *
...
/**
 * Hành vi cũ cho deep link KHÔNG kèm sid (AMIS bản cũ): chỉ ép link
 * `misajsc.amis.vn/lms` về trang đăng nhập SaaS, host khác giữ nguyên.
 *
 * Giữ phạm vi hẹp này có chủ đích — mở rộng cho mọi host sẽ đổi hành vi của
 * những deep link đang chạy (vd site không cài plugin `auth_saas` thì
 * `/auth/saas/index.php` sẽ 404).
 */
